...
def plot_tops_and_bottom(df : pd.DataFrame, tops=None, bottoms=None, mode="close"):
    ...
    if mode == "close":
        fig = px.line(df, x=idx, y="Close", title="Closing prices")
    elif mode == "candle":
    ...
    fig.add_scatter(x=[idx[top[1]] for top in tops], y=[top[2] for top in tops], mode="markers", marker=dict(color='green', size=10), name='Tops')
    fig.add_scatter(x=[idx[bottom[1]] for bottom in bottoms], y=[bottom[2] for bottom in bottoms], mode="markers", marker=dict(color='red', size=10), name='Bottoms')
...
def main():
    ...
    fig = plot_tops_and_bottom(data, tops, bottoms, mode="candle")
